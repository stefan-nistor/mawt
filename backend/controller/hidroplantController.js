...
module.exports.createHidroplant = async (req, res) => {
  ...
  try {
    ...
    if (hidroplant) {
      res.statusCode = 403;
      res.write(
        JSON.stringify({ success: false, messageL: "name has been taken" })
      );
      res.end();
    } else {
...
module.exports.updateHidroplant = async (req, res) => {
  ...
  try {
    ...
    if (hidroplant) {
      const hidroplantUpdate = await req.db.Hidroplant.updateOne(
        { _id: id },
        {
          $set: {
            ...
            main_basin: req.body.main_basin
              ? req.body.main_basin
              : hidroplant.main_basin,
            catch_area: req.body.catch_area
              ? req.body.catch_area
              : hidroplant.catch_area,
            op_status: req.body.op_status
              ? req.body.op_status
              : hidroplant.op_status,
            commisioned: req.body.commisioned
              ? req.body.commisioned
              : hidroplant.commisioned,
            dam_completed: req.body.dam_completed
              ? req.body.dam_completed
              : hidroplant.dam_completed,
            no_units: req.body.no_units
              ? req.body.no_units
              : hidroplant.no_units,
            dam_heigth: req.body.dam_heigth
              ? req.body.dam_heigth
              : hidroplant.dam_heigth,
            min_wHeadHgt: req.body.min_wHeadHgt
              ? req.body.min_wHeadHgt
              : hidroplant.min_wHeadHgt,
            max_wHeadHgt: req.body.max_wHeadHgt
              ? req.body.max_wHeadHgt
              : hidroplant.max_wHeadHgt,
            res_capacity: req.body.res_capacity
              ? req.body.res_capacity
              : hidroplant.res_capacity,
            res_area: req.body.res_area
              ? req.body.res_area
              : hidroplant.res_area,
            elec_cap: req.body.elec_cap
              ...
              : hidroplant.elec_cap,
            transm_exist: req.body.transm_exist
              ? req.body.transm_exist
              : hidroplant.transm_exist,
            transm_length: req.body.transm_length
              ? req.body.transm_length
              : hidroplant.transm_length,
            transm_planned: req.body.transm_planned
              ? req.body.transm_planned
              : hidroplant.transm_planned,
            type: req.body.type ? req.body.type : hidroplant.type,
            ann_firm_gen: req.body.ann_firm_gen
              ? req.body.ann_firm_gen
              : hidroplant.ann_firm_gen,
            ann_total_gen: req.body.ann_total_gen
              ? req.body.ann_total_gen
              : hidroplant.ann_total_gen,
      ...
      res.setHeader("Content-type", "application/json");
      res.write(
        JSON.stringify({ success: true, hidroplantUpdate, message: "UPDATED" })
      );
      res.end();
    } else {
      res.statusCode = 404;
      ...
      res.write(
        JSON.stringify({
          success: false,
          message: "no hidroplant with this id",
        })
      );
      res.end();
    }
...
module.exports.deleteHidroplant = async (req, res) => {
  ...
  try {
    ...
    if (hidroplant) {
      ...
      res.setHeader("Content-type", "application/json");
      res.write(
        JSON.stringify({ success: true, message: "removed hidroplant" })
      );
      res.end();
    } else {
      res.statusCode = 404;
  ...
  try {
    ...
    if (hidroplant) {
      ...
      res.write(
        JSON.stringify({
          success: false,
          message: "no hidroplant for this name",
        })
      );
      res.end();
    }
...
module.exports.getTopNHidroplants = async (req, res) => {
  ...
  try {
    ...
    const hidroplantsFromDb = await req.db.Hidroplant.find().sort({
      elec_cap: -1,
    });
    ...
    res.write(
      JSON.stringify({
        success: true,
        hidroplants,
        message: `Successfully retrieved the first ${howMany} hidroplants`,
      })
    );
    res.end();
  } catch (e) {
  ...
  try {
    ...
    res.write(
      JSON.stringify({
        success: true,
        closestHidroplant,
        message: `Retrieved hidroplant ${closestHidroplant.name}`,
      })
    );
    res.end();
  } catch (e) {
  ...
  try {
    ...
    res.write(
      JSON.stringify({
        success: false,
        hidroplant,
        message: "current electric capacity of the hidroplant",
      })
    );
    res.end();
  } catch (e) {
