...
const addHidropowerList = async() => {
    ...
    for (let counter = 0; counter < hidroplants.length; counter++) {
        ...
        let item = `<div id="${hidroplants[counter].name}">
                      <a href="./hidroPower-Page.html?name=${hidroplants[counter].name}">
                          <div class="hidro-one">
                              <div class="white-box">
                                  ${counter + 1}
                              </div>
    ...
    for (const trashBtn of trashButtons) {
        trashBtn.addEventListener('click', async() => {
            try {
                ...
                const res = await axios.delete(`${BASE_URL}/hidroplants/${hidroplant._id}`, {
                    headers: {
                        Authorization: `Bearer ${authToken}`
                    },
                    data: {}
