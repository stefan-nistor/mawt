...
const addHidropowerList = async() => {
    ...
    for (let counter = 0; counter < hidroplants.length; counter++) {
        ...
        let item = `<a href="./hidroPower-Page.html?name=${encodeURIComponent(hidroplants[counter].name)}">
                        <div class="hidro-one">
                            <div class="white-box">
                                ${counter + 1}
                            </div>
...
const addHidropowerListAdmin = async() => {
    ...
    for (let counter = 0; counter < hidroplants.length; counter++) {
        const description = hidroplants[counter].purpose ? hidroplants[counter].purpose : "Hidroplant";
        let item = `<div id="${hidroplants[counter].name}" style="text-decoration:none">
                      <a href="./hidroPower-Page.html?name=${encodeURIComponent(hidroplants[counter].name)}">
                          <div class="hidro-one">
                              <div class="white-box">
                                  ${counter + 1}
                              </div>
